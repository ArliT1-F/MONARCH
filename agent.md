...
- `player.ts` — `MusicManager`: per-guild AudioPlayer + VoiceConnection driven by the pure queue. Idle handler advances (loop modes decide); `skipping`/`stopping` flags distinguish manual stop from natural end; 3 consecutive failures -> give up + teardown; empty channel -> leave after 60s; idle -> leave after 5min. Announcements post to the last music command's text channel. Volume 0-150 via `resource.volume` (**needs ffmpeg** — `ffmpeg.ts` resolves FFMPEG_PATH -> @ffmpeg-installer/ffmpeg -> system; Docker image ships the apk).
  - **Voice join:** `connect()` never reuses a connection that is destroyed/disconnected/pointed at another channel (a timed-out join used to leave the session claiming to be in voice, so the next `/music play` skipped joining and silently played nothing). The connection gets `error` (unhandled `error` on that EventEmitter would take the worker down), `stateChange` (logged at debug) and `Destroyed` listeners; `teardown()` deletes the session *before* `destroy()` so the Destroyed handler can't re-enter it.
  - **Join failures are explained:** `entersState(Ready, 20s)` only rejects with a bare `AbortError: The operation was aborted`, so `describeVoiceFailure()` reads the connection's own state (voice `status`, the `NetworkingStatusCode` sub-state, the websocket `closeCode`) and throws a `VoiceError` whose message names the stalled stage — UDP handshake (= host blocks outbound UDP, voice is UDP-only), voice gateway handshake, encryption negotiation, or the close code (4014 = missing Connect/Speak). The technical fields plus `generateDependencyReport()` (ffmpeg/opus/encryption/DAVE) land in one `voice connection failed` error line, and the failed session is torn down so a retry re-joins. `/music` shows `VoiceError` messages verbatim (same path as `SourceError`).
