 ...
 */
export default async function AnalyzerPage({ params }: { params: Promise<{ guildId: string }> }) {
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-6 sm:px-8 sm:py-10">
        ...
        <div className="rounded-2xl border border-warn-400/20 bg-warn-400/5 p-4 text-sm text-warn-400 sm:p-5">
          ...
          {guild.botInstalled
            ? " (Discord didn't respond — try again shortly)."
            : " — the Monarch bot isn't installed in this server."}
        </div>
        {!guild.botInstalled && (
          <div className="mt-4">
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6 sm:px-8 sm:py-10">
      ...
      <p className="mb-6 text-sm leading-relaxed text-ink-300 sm:mb-8">
        ...
        concrete suggestions. Nothing here changes your server; apply structure changes through the
        Server Designer.
      </p>
      <AnalyzerPanel
        guildId={guildId}
