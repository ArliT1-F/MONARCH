 ...
 */
export default async function HelpPage({ params }: { params: Promise<{ guildId: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6 sm:px-8 sm:py-10">
      ...
      <p className="mb-8 max-w-2xl text-sm leading-relaxed text-ink-300 sm:mb-6">
        Everything Monarch can do in Discord — the design studio, the burg gag, and the music player
        — with every option, who can run it and what it needs. Every command works two ways: as a
        slash command (
        <code className="rounded bg-ink-900 px-1.5 py-0.5 text-xs text-gold-400">
          /monarch help
        </code>
        ) or as a plain message with your server&apos;s prefix (
        <code className="rounded bg-ink-900 px-1.5 py-0.5 text-xs text-gold-400">!help</code>,
        changeable with{" "}
        <code className="rounded bg-ink-900 px-1.5 py-0.5 text-xs text-gold-400">
          !prefix set ?
        </code>
        ). Type either one in Discord for the short version of this page.
      </p>
