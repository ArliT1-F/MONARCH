...
if [[ -d "$REPO/apps/bot/node_modules" || -d "$REPO/node_modules" ]]; then
  ...
  # @discordjs/voice has no built-in codec. apps/bot/src/music/player.ts runs
  # StreamType.Arbitrary + inlineVolume, so every frame goes ffmpeg → PCM →
  # Opus and an encoder is mandatory, on every host, UDP or not.
