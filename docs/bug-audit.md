...
`apps/bot/src/music/lavalink.ts` (869 lines) made the bot a _client_ of a JVM
...
`.env.example` — a _tracked_ file — contained a real Neon connection string:
...
(_Project → Roles → Reset password_), then update `DATABASE_URL` wherever it is
...
| 8   | On the Opus-passthrough path, a _failed download_ was reported to the user as "install ffmpeg", and `createAudioResource` could surface prism-media's raw `FFmpeg/avconv not found!`.         | yt-dlp's own failure is checked and reported first; resource creation is wrapped in `AudioError` with a readable message.              |
...
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--http-chunk-size 16384` (new)                             | YouTube throttles a _connection_, not an account: one long download starts fast, then crawls. Requesting 16 KiB ranges turns it into many short requests that each come back at full speed. Verified byte-identical on the stdout path, and harmless for servers without range support. |
...
| `--retries 5`, `--fragment-retries 5` (verified)            | yt-dlp resumes a _stdout_ download from the byte it last wrote (`ctx.resume_len = byte_counter`), so an interruption reopens a range request instead of starting over or duplicating audio.                                                                                             |
