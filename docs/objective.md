...
# APPENDIX I — Hardening session (2026-09-25)

> **Status:** completed in this session. No product surface added; this is the
> engineering-hygiene pass §28 (TESTING) and §34 (CODE QUALITY) asked for and
> the repo had not yet got. Implementation detail lives in `agent.md`
> ("Hardening session (2026-09-25)"); this records the decisions.

| #   | Finding (verified, not assumed)                                                                                                                                                                                                                                              | Change                                                                                                                                                                                                                                                                                                                                                                                                                |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | No CI: the repo had 560+ unit tests, a strict `tsc`, and nothing that ran either.                                                                                                                                                                                            | `.github/workflows/ci.yml`: install → `db:generate` → typecheck → lint → format → test → production build → boot + header assertion; second job for `docker compose config -q`.                                                                                                                                                                                                                                       |
| 2   | `.prettierrc.json` + `.prettierignore` existed, but Prettier was not a dependency and no script invoked it.                                                                                                                                                                  | Prettier added as a devDependency; `format` / `format:check`; the tree was already conformant, so enforcement is free.                                                                                                                                                                                                                                                                                                |
| 3   | Six `eslint-disable` comments named rules no config defined (`@next/next/no-img-element`, `react-hooks/exhaustive-deps`) — suppressions decayed in silence, and Next's build-time lint errors on unknown rules.                                                              | ESLint 9/10 flat config with both plugins registered (those two rules at `warn`), `no-console` for the redacting logger, `no-restricted-imports` for the §14 gateway seam, and `no-floating-promises` / `no-misused-promises` type-aware on `apps/bot/src` only. `next.config.ts` sets `eslint.ignoreDuringBuilds`: lint belongs in its own step.                                                                     |
| 4   | Lint's first pass found 22 real issues, incl. a 22-line dead `chunkLines` helper in `apps/bot/src/commands.ts` with two no-op `.map()` calls, and two unreachable `let json = null` initialisers in the template upload routes.                                              | Fixed, not suppressed.                                                                                                                                                                                                                                                                                                                                                                                                |
| 5   | `addSnapshot` appended forever in both stores while every apply writes two snapshots; the file store rewrites the whole `snapshots.json` on each add (audit rows were capped at 2000, snapshots not at all).                                                                 | `apps/dashboard/lib/retention.ts` — pure rule, called inside both stores: 25 newest per automatic kind, `manual` never, 60 s grace so an apply cannot eat its own snapshot.                                                                                                                                                                                                                                           |
| 6   | The dashboard sent no security headers at all.                                                                                                                                                                                                                               | CSP, `nosniff`, `X-Frame-Options`/`frame-ancestors 'none'`, `Referrer-Policy`, `Permissions-Policy`, COOP, `poweredByHeader: false`. Verified by booting `next start` and reading the response, not by reading the config.                                                                                                                                                                                            |
| 7   | `shutdown()` did `try { client.destroy(); } catch { log.warn("gateway close failed") }` then `process.exit(0)` — `destroy()` is async, so the catch could never see a rejection and the exit never waited for the close. Same shape in the disallowed-intents recovery path. | Both awaited (the shutdown one through the existing `capWait`, so a wedged close still cannot outlive `TimeoutStopSec`). `shutdown.test.ts` now models `destroy()` as the async method it is; `signal()` drains the queue since the handler is `void shutdown(…)`. Mutation-checked: reverting the `await` fails exactly that test.                                                                                   |
| 8   | A fresh clone that cannot reach `binaries.prisma.sh` had `npm install` exit 1 (postinstall `prisma generate`) and `npm test` report 5 erroring suites.                                                                                                                       | `scripts/ensure-prisma-client.mjs` fails loudly in CI and in a container build (both Dockerfiles generate the client through this hook) and warns-and-continues elsewhere; `vitest.config.ts` derives which suites import the generated client and filters those out with a printed notice; `MONARCH_REQUIRE_CODEGEN=1` makes the same situation an error in CI, so a green run there proves the database suites ran. |
| 9   | `agent.md` §7 claimed 7 remaining `apps/bot` typecheck errors and a 497/11 test split; `tsc` is clean and the suite is bigger than that.                                                                                                                                     | Documentation refreshed to the measured state (571 tests / 41 files here, 46 files with codegen).                                                                                                                                                                                                                                                                                                                     |

Still open, unchanged by this session: UI/component tests (§28, Appendix C
item 8), the formal feature-module registry (§24, Appendix C item 7), CSP
nonces (`csp: { nonce: true }` + middleware — needs a clicked-through build),
per-guild single-flight on `POST /api/guilds/:id/apply` (two admins applying
at once can still double-create), and rate limiting on the dashboard's send
routes. Type-aware linting across all workspaces is deliberately not enabled:
it wants a task graph to stay fast, and `tsc --noEmit` already owns types.
