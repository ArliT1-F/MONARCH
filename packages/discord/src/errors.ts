...
export function translateDiscordError(e: unknown, context: string): MonarchError {
  ...
  if (status === 403 || code === 50013) {
    return monarchError("discord.permissions", `Monarch doesn't have permission to ${context}.`, {
      reason:
        "The Monarch bot is missing the required permission, or the resource sits above its highest role.",
  ...
  if (status === 404 || code === 10003) {
    return monarchError(
      "discord.not-found",
      `Monarch couldn't find the resource needed to ${context}.`,
      {
  ...
  if (status === 429) {
    return monarchError(
      ...
      `Discord rate-limited Monarch while trying to ${context}.`,
      {
